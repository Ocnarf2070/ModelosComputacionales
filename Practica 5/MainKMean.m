...
Parameter.NumNeu=[Neu(1) 100 50 25 Neu(2)]; 


Parameter.Eta=0.1;
Parameter.NumEpoch=200;
...
NF=FilData;
prompt = sprintf('Introduce the K value for the K-mean (K<%d):\n',floor(NF/4));
K=input(prompt);
for j=1:ceil(NF/K)
    
    FilaTrain=NF-(j-1)*K-mod(NF,K);
    
    Datos.Train=[Data2(1:FilaTrain,:);Data2(FilaTrain+K+1:end,:)];
     if j==1
       Datos.Tests=Data2(FilaTrain+1:end,:);
     else
        Datos.Tests=Data2(FilaTrain+1:FilaTrain+K,:);
     end
     n=size(Datos.Train,1);
     m=mod(n,K);
     if m==0
        FilaValid=n-3*K+1; 
     else
     ...
     Datos.Valid=Datos.Train(FilaValid:end,:);
     Datos.Train=Datos.Train(1:FilaValid-1,:);
    ...
    if j==1
        ...
        MSE.PC.Prob=mean([MSE.PC.Prob, BP_Calcular_Prob(Network_PCFinal, Datos.Tests,Parameter)]);
    end
end
